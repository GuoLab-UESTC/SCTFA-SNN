...
class snn(nn.Module):
    def __init__(self, sctfa):
        ...
        in_planes, out_planes, stride, padding, kernel_size = conv_kernal[0]
        self.conv1 = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding)
        self.bn1 = nn.BatchNorm2d(out_planes)
        if self.sctfa:
            self.att1 = attention(out_planes, out_planes)

        in_planes, out_planes, stride, padding, kernel_size = conv_kernal[1]
        self.conv2 = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding)
        self.bn2 = nn.BatchNorm2d(out_planes)
        if self.sctfa:
            self.att2 = attention(out_planes, out_planes)

        in_planes, out_planes, stride, padding, kernel_size = conv_kernal[2]
        ...
        self.fc1 = nn.Linear(conv_kernal[-1][0] * conv_feature_size[-1][1] * conv_feature_size[-1][1],
                             fc_feature_size[0])
        self.fc2 = nn.Linear(fc_feature_size[0], fc_feature_size[1] * 5)

    def forward(self, inputs, time_window=20):
        batch_size = inputs.size(0)
        c1_spike = c1_mem = torch.zeros(batch_size, conv_kernal[0][1], conv_feature_size[0][0], conv_feature_size[0][1],
                                        device=device)
        c2_spike = c2_mem = torch.zeros(batch_size, conv_kernal[1][1], conv_feature_size[1][0], conv_feature_size[1][1],
                                        device=device)
        c3_spike = c3_mem = torch.zeros(batch_size, conv_kernal[2][1], conv_feature_size[2][0], conv_feature_size[2][1],
                                        device=device)

        h1_mem = h1_spike = torch.zeros(batch_size, fc_feature_size[0], device=device)
        h2_mem = h2_spike = h2_sumspike = torch.zeros(batch_size, fc_feature_size[1] * 5, device=device)
